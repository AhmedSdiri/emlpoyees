...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('client-management.create') }}">Add new client</a>
        </div>
    </div>
  </div>
  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('client-management.search') }}">
         {{ csrf_field() }}
         @component('layouts.search', ['title' => 'Search'])
          @component('layouts.two-cols-search-row', ['items' => ['First Name','LastName','Tel'], 
            ...
            <thead>
             ...
             <tbody>
                ...
                <tr role="row" class="odd">
                  <td><img src="../{{$client->picture }}" width="50px" height="50px"/></td>
                  <td class="sorting_1">{{ $client->firstname }} {{$client->lastname}}</td>
                  <td class="hidden-xs">{{ $client->email }}</td>
                  <td class="hidden-xs">{{ $client->tel }}</td>
